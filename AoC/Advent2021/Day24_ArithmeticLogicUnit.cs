...
public class Day24 : IPuzzle
{
    ...
    private static long FindModelNumber(bool biggest)
    {
        ...
        while (queue.TryDequeue(out (int prevZ, int digit) state, out var _))
        {
            for (int inputW = 10 - 1; inputW >= 1; --inputW)
            {
                ...
                if (!cache.TryGetValue(cacheKey, out (int w, int z) cached) || (biggest && cached.w < inputW) || (cached.w > inputW))
                {
    ...
    private static long ReconstructAll(Dictionary<int, (int w, int z)> cache, HashSet<(int prevZ, int finalW)> endStates) => endStates.Select(v => v.finalW + Reconstruct(cache, v.prevZ, 12)).Max();
    ...
    public static long Part2(string _) => FindModelNumber(biggest: false);
