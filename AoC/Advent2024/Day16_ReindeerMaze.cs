...
{
    static IEnumerable<((int, int) newPos, char newDir, int cost)> GetMoves(HashSet<(int, int)> walkable, (int, int) pos, Direction2 d)
    {
    ...
    private static (int best, int visited) Solve(string input)
    {
        return Memoize(input, _ =>
        {
            ...
            var start = grid.KeysWithValue('S').Single();
            var end = grid.KeysWithValue('E').Single();
